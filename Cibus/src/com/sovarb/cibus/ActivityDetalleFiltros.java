...
public class ActivityDetalleFiltros extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_detalle_filtros);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data){
	    ...
	    }else if(requestCode == S.REQUEST_CODE_2 && resultCode != RESULT_OK){
			finish();
		
	    }else if (requestCode == S.REQUEST_CODE_2 && resultCode == RESULT_OK){
			String nombreExtra, descripcionExtra;
			if((nombreExtra = data.getStringExtra(S.NOMBRE_FILTRO)) != null){
				nombre.setText(nombreExtra);
			}
			if((descripcionExtra = data.getStringExtra(S.DESCRIPCION_FILTRO)) != null){				
				descripcion.setText(descripcionExtra);
			}
			cargarReglas();
		}else if(requestCode != S.REQUEST_CODE_3){
	...
	public void pulsaEditar(View view){	
		...
		Intent i = new Intent(this, ActivityEditarFiltros.class);
		i.putExtra(S.NOMBRE_FILTRO, nombre.getText().toString());
		i.putExtra(S.DESCRIPCION_FILTRO, descripcion.getText().toString());
		startActivityForResult(i, S.REQUEST_CODE_2);
	}
	...
	private void cargarReglas(){
		...
		S.data_base.openR();
		ArrayList<Regla> reglas = S.data_base.consultaReglasXFiltro(nombre.getText().toString());
		S.data_base.close();
	...
	private final S.RESULTADO filtrarRegla(String pRegla, String[] pIngredientes, String[] pAditivos){
		...
		if(pIngredientes.length == 0 && pAditivos.length == 0){	return S.RESULTADO.SIN_DATOS;	}
